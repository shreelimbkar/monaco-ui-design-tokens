...
export function setupColorTokens(colorFrame, format) {
  if (colorFrame) {
    ...
    colorFrame.children.forEach(color => {
      ...
      contrastColor = `rgba(${roundColorValue(
        color.children[2].fills[0].color.r,
        255
      )}, ${roundColorValue(
        color.children[2].fills[0].color.g,
        255
      )}, ${roundColorValue(
        color.children[2].fills[0].color.b,
        255
      )}, ${roundColorValue(color.children[2].fills[0].color.a, 255)})`;

      if (color.children.length === 5) {
        lightRatio = color.children[4].name;
        lightCompliance = color.children[3].name;
        lightColor = `rgba(${roundColorValue(
          color.children[3].fills[0].color.r,
          255
        )}, ${roundColorValue(
          color.children[3].fills[0].color.g,
          255
        )}, ${roundColorValue(
          color.children[3].fills[0].color.b,
          255
        )}, ${roundColorValue(color.children[3].fills[0].color.a, 255)})`;
      }

      if (format == "js") {
        colorString = `rgba(${roundColorValue(
          color.background[0].color.r,
          255
        )}, ${roundColorValue(
          color.background[0].color.g,
          255
        )}, ${roundColorValue(
          color.background[0].color.b,
          255
        )}, ${roundColorValue(color.background[0].color.a, 1)})`;
      } else {
        if (color.children.length === 5) {
          colorString = {
            value: `rgba(${roundColorValue(
              color.background[0].color.r,
              255
            )}, ${roundColorValue(
              color.background[0].color.g,
