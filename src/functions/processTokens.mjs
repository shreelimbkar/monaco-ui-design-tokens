...
import { setupKerningTokens } from "./setupKerningTokens.mjs";
import { setupDroidKerningTokens } from "./setupDroidKerningTokens.mjs";
...
export function processTokens(sheet, name, format) {
  if (sheet && name) {
    ...
    if (_name === "spacing" || _name === "spacings" || _name === "size") {
    ...
    if (_name === "kerning" || _name === "kernings") {
      processedTokens = setupKerningTokens(sheet, format);
    }
    if (_name === "droidkerning" || _name === "droidkernings") {
      processedTokens = setupDroidKerningTokens(sheet, format);
    }
